...
if __name__ == "__main__":
	...
	""" Initialize parameters
		...
		m_score = multiply-by-score
		ckpt_fn = checkpoints for the inference layer
	"""
	m_score		= 	rospy.get_param('~m_score', 'True')
	debug 		= 	rospy.get_param('~debug', 'False')
	ckpt_fn		= 	rospy.get_param('~ckpt', ' ')
	...
	model 	= 	Model(input_size=input_size, num_layers=num_layers, hidden_size=hidden_size, \
						seq_len=seq_len, num_features=num_features, dropout=dropout, mode="TEST")
